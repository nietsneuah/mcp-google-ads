...
async def update_campaign_bidding(
    ...
    bidding_strategy: str = Field(
        description="Bidding strategy: MAXIMIZE_CONVERSIONS, MAXIMIZE_CONVERSION_VALUE, MANUAL_CPC"
    ),
    target_cpa_micros: int = Field(
        default=None,
        description="Target CPA in micros (e.g., 100000000 = $100). Only for MAXIMIZE_CONVERSIONS.",
    ),
    target_roas: float = Field(
        default=None,
        description="Target ROAS as decimal (e.g., 1.5 = 150%). Only for MAXIMIZE_CONVERSION_VALUE.",
    ),
) -> str:
    """
    Update the bidding strategy of an existing campaign.

    Change between Maximize Conversions, Maximize Conversion Value, or Manual CPC.
    Optionally set target CPA (for Max Conversions) or target ROAS (for Max Conversion Value).

    Args:
        customer_id: The Google Ads customer ID as a string (10 digits, no dashes)
        campaign_resource_name: The resource name of the campaign to update
        bidding_strategy: One of MAXIMIZE_CONVERSIONS, MAXIMIZE_CONVERSION_VALUE, MANUAL_CPC
        target_cpa_micros: Optional target CPA in micros (only for MAXIMIZE_CONVERSIONS)
        target_roas: Optional target ROAS as decimal (only for MAXIMIZE_CONVERSION_VALUE)
    ...
    Example:
        ...
        campaign_resource_name: "customers/4001027289/campaigns/22203438637"
    ...
    try:
        ...
        update_fields = {"resourceName": campaign_resource_name}
        update_mask_parts = []
        ...
        if strategy == "MAXIMIZE_CONVERSIONS":
            if target_cpa_micros is not None:
                ...
                update_mask_parts.append("maximize_conversions.target_cpa_micros")
            else:
                update_fields["maximizeConversions"] = {}
                update_mask_parts.append("maximize_conversions")
        elif strategy == "MAXIMIZE_CONVERSION_VALUE":
            if target_roas is not None:
                update_fields["maximizeConversionValue"] = {
                    "targetRoas": target_roas
                }
                update_mask_parts.append("maximize_conversion_value.target_roas")
            else:
                update_fields["maximizeConversionValue"] = {}
                update_mask_parts.append("maximize_conversion_value")
        elif strategy == "MANUAL_CPC":
            update_fields["manualCpc"] = {}
            update_mask_parts.append("manual_cpc")
        else:
            return f"Error: Unknown bidding strategy '{bidding_strategy}'. Use MAXIMIZE_CONVERSIONS, MAXIMIZE_CONVERSION_VALUE, or MANUAL_CPC."
        ...
        if response.status_code != 200:
            return f"Error updating campaign bidding: {response.text}"

        result = response.json()
        if result.get("results"):
            details = f"Strategy: {strategy}"
            if target_cpa_micros is not None:
                details += f"\nTarget CPA: ${target_cpa_micros / 1000000:.2f}"
            if target_roas is not None:
                details += f"\nTarget ROAS: {target_roas * 100:.0f}%"
            return f"Successfully updated campaign bidding\nCampaign: {campaign_resource_name}\n{details}"
        else:
            return f"Campaign bidding update completed: {result}"

    except Exception as e:
        return f"Error updating campaign bidding: {str(e)}"


@mcp.tool()
async def update_shared_set_members(
    customer_id: str = Field(
        description="Google Ads customer ID (10 digits, no dashes)"
    ),
    shared_set_resource_name: str = Field(
        description="Shared set resource name (e.g., customers/123/sharedSets/456)"
    ),
    add_keywords: list = Field(
        default=None,
        description="Keywords to add: list of {text, match_type} objects. match_type: EXACT, PHRASE, or BROAD",
    ),
    remove_resource_names: list = Field(
        default=None,
        description="Shared criteria resource names to remove (e.g., customers/123/sharedCriteria/456~789)",
    ),
) -> str:
    """
    Add or remove keywords from a shared negative keyword list.

    Use this to manage shared negative keyword lists that are applied across campaigns.
    You can add new keywords, remove existing ones, or both in a single call.

    Args:
        customer_id: The Google Ads customer ID as a string (10 digits, no dashes)
        shared_set_resource_name: The resource name of the shared set
        add_keywords: Optional list of keyword dicts with 'text' and 'match_type'
        remove_resource_names: Optional list of shared criteria resource names to remove
    ...
    Example:
        customer_id: "4001027289"
        shared_set_resource_name: "customers/4001027289/sharedSets/11906638858"
        add_keywords: [{"text": "dry cleaners", "match_type": "PHRASE"}]
        remove_resource_names: ["customers/4001027289/sharedCriteria/11906638858~42743030"]
    """
    try:
        creds = get_credentials()
        headers = get_headers(creds)
        ...
        formatted_customer_id = format_customer_id(customer_id)
        url = f"https://googleads.googleapis.com/{API_VERSION}/customers/{formatted_customer_id}/sharedCriteria:mutate"
        ...
        if remove_resource_names:
            for resource_name in remove_resource_names:
                operations.append({"remove": resource_name})

        if not operations:
            return "Error: No operations specified. Provide add_keywords or remove_resource_names."

        payload = {"operations": operations}
        response = requests.post(url, headers=headers, json=payload)

        if response.status_code != 200:
            return f"Error updating shared set: {response.text}"

        result = response.json()
        if result.get("results"):
            added = len(add_keywords) if add_keywords else 0
            removed = len(remove_resource_names) if remove_resource_names else 0
            output = f"Successfully updated shared set: {shared_set_resource_name}\n"
            output += f"Added: {added} keywords, Removed: {removed} keywords\n"
            output += "------------------------------------------------------------\n"
            for i, res in enumerate(result["results"]):
                output += f"{i + 1}. {res.get('resourceName', 'completed')}\n"
            return output
        else:
            return f"Shared set update completed: {result}"
